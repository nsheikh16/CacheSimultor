...
char *make_block (int block_size) {
   ...
   */
      ...
      char* mb = (char*) (malloc(sizeof(char)*(block_size)));
      return mb;
}
...
Line *make_lines (int line_count, int block_size) {
   ...
   */
     Line* lc = (Line*) malloc(sizeof(Line)*(line_count));
     for (int i=0;i<line_count; i++){
     lc[i].accessed=make_block(block_size);
     lc[i].block_size=block_size;
   }
...
Set *make_sets (int set_count, int line_count, int block_size) {
   ...
   */
     Set* sc =  (Set*)malloc(sizeof(Set)*(set_count));
     for (int i=0;i<set_count; i++){
       sc[i].lines=make_lines(line_count,block_size);
       sc[i].line_count=line_count;
